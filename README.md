...
本体ゲートウェイは仕様策定段階です。この確認用ブランチでは`src/main.cpp`にPA0のPWM（2秒上昇/2秒下降）とPA3の1秒反転を実装しています。下記のゲートウェイ機能は未実装です。確認プログラムはC++20でビルドしていますが、本体全体のツールチェーン適合は今後の検証対象です。手順と限界は [PWM/DO書込み確認](docs/pwm-do-smoke.md) を参照してください。
...
| 設定・診断 | 揮発設定、入力フィルタ、ADC異常時の非常ブレーキ、通信監視、安全出力、LED/専用ログ。Releaseは起動・状態変化・異常を記録 |

シミュレータ用途の試作を対象とします。負荷駆動回路、ESP32マスターのソフトウェア、CAN FD、Flashへの設定保存、独自更新ブートローダは今回の実装範囲に含めません。電気的な対象はMCUの3.3 V論理信号までです。EVTの公式資料を先行調査し、実機の基板版・ジャンパー・導通は別途確認します。カスタム基板詳細は後工程、USB VID/PIDは指定なし・選定待ちです。接続前に [インターフェース仕様](docs/interface-spec.md) と [未決事項](docs/decisions.md#未決事項台帳) を確認してください。

ソフト受入の連続試験は24時間です。温度試験は対象外で、0～40℃は基板側の未検証前提として別管理します。CANは少数ノード・短距離を先行し、最大16ノード/40 mは要求を保持して設備待ち、ESP32マスターとの結合は別プロジェクト待ちです。これらは今後の試験計画であり、実施済みの保証ではありません。
...
`test/test_pwm_do_smoke/`にはWindows/MSVC用のUnityホスト試験があります。本体用のPlatformIOホスト試験環境は未構築です。予定する検証は [試験計画](docs/test-plan.md) を参照してください。組み込み環境の`pio test`は書き込みを伴う場合があるため、ビルドだけの確認として実行しません。
...
作業前に`AGENTS.md`とprogressを読み、関連仕様とGit差分を確認します。仕様・配置の変更はREADME、AGENTS、関連文書の整合を保ち、必要な進捗や検証根拠を個人情報・機密情報を除いて随時ファイルへ残します。

コミットには個人情報・機密情報を含めません。ユーザー名入りフルパス、秘密鍵、パスワード、トークン等は、文書・ログ・画像・試験データ・ファイル名・コミットメッセージにも記録しません。パスはリポジトリ相対または`${env:USERPROFILE}`等の環境変数で表し、保存前に出力を匿名化します。毎回ステージ済み内容全体を確認し、既存履歴に見つかった場合は値を転載せず報告します。詳細は [運用規約](docs/development-workflow.md) に従います。

影響範囲を小さくするため、1つの意味ある作業の最小単位で検証・コミットします。1プロンプトを1コミットにまとめる必要はありません。独立した修正・調査は分け、一緒でなければ整合しない変更は同じ単位に保ちます。メッセージは`add gpio tests`のような短い小文字の命令形にします。
